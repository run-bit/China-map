...
function draw(data) {
  ...
  const width = mapRoot.node().clientWidth - 24, height = 560;
  svg = mapRoot.append("svg").attr("viewBox", `0 0 ${width} ${height}`).attr("preserveAspectRatio", "xMidYMid meet");
  group = svg.append("g");
  const projection = d3.geoMercator().fitExtent([[18, 18], [width - 18, height - 18]], data);
...
fetch(GEOJSON_URL).then(r => { if (!r.ok) throw new Error(); return r.json(); }).then(draw).catch(() => {
